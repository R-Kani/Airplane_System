	...
	for pilih != 8 {
		...
		if pilih == 1 {
			...
			if pilih2 == 1 {
				...
				y := mencariOrangTua(x.np_ortu)
				fmt.Print("\n========================================================\n")
				fmt.Print("\nNama\t\t\t: " + y.nama +
					"\nKelas\t\t\t: " + y.kelas_penerbangan +
					"\nNo. Reservasi\t\t: " + strconv.Itoa(y.kode_reservasi) +
					"\nNo. Paspor\t\t: " + y.np +
					"\nNo. Paspor Orang Tua\t: " + y.np_ortu +
					"\nUsia\t\t\t: " + strconv.Itoa(y.usia) +
					"\nVegetarian\t\t: " + y.diet +
					"\nBaris, No. Kursi\t: " + strconv.Itoa(y.tempatDuduk.baris) + ", " + strconv.Itoa(y.tempatDuduk.kolom) + "\n" +
					"\n========================================================\n")
...
func reservasi() {
	...
	*/
	...
	if kelas == "Bisnis" {
		...
		fmt.Print("Jumlah Penumpang (Umur >= 2 tahun): ")
		fmt.Scan(&dewasa)
		fmt.Print("Jumlah Penumpang Bayi (Umur < 2 tahun): ")
		fmt.Scan(&bayi)
		jumlah := dewasa + bayi
		jumlah_tanpa_bayi := dewasa
		if jumlah <= T {
	...
	*/
	...
	if kelas_penerbangan == "Bisnis" {
		...
		for i := 0; i < len(array_penumpang[kode_reservasi-1]); i++ {
			if array_penumpang[kode_reservasi-1][i].usia >= 2 {
				array_penumpang[kode_reservasi-1][i].tempatDuduk = kursi{baris: listKursi[i].baris, kolom: listKursi[i].kolom}
				plane.Bisnis[listKursi[i].baris][listKursi[i].kolom] = array_penumpang[kode_reservasi-1][i]
				if array_penumpang[kode_reservasi-1][i].usia >= 18 {
					kursi_ortu = array_penumpang[kode_reservasi-1][i].tempatDuduk
				}
			} else {
				array_penumpang[kode_reservasi-1][i].tempatDuduk = kursi_ortu

			}
			fmt.Print("\nNama\t\t\t: " + array_penumpang[kode_reservasi-1][i].nama +
				"\nKelas\t\t\t: " + array_penumpang[kode_reservasi-1][i].kelas_penerbangan +
				"\nNo. Reservasi\t\t: " + strconv.Itoa(array_penumpang[kode_reservasi-1][i].kode_reservasi) +
				"\nNo. Paspor\t\t: " + array_penumpang[kode_reservasi-1][i].np +
				"\nNo. Paspor Orang Tua\t: " + array_penumpang[kode_reservasi-1][i].np_ortu +
				"\nUsia\t\t\t: " + strconv.Itoa(array_penumpang[kode_reservasi-1][i].usia) +
				"\nVegetarian\t\t: " + array_penumpang[kode_reservasi-1][i].diet +
				"\nBaris, No. Kursi\t: " + strconv.Itoa(array_penumpang[kode_reservasi-1][i].tempatDuduk.baris) + ", " + strconv.Itoa(array_penumpang[kode_reservasi-1][i].tempatDuduk.kolom) + "\n" +
				"\n========================================================\n")
	...
	if kelas_penerbangan == "Ekonomi" {
		listKursi := kursiKosong(kelas_penerbangan)
		fmt.Print("\n========================================================\n")
		for i := 0; i < len(array_penumpang[kode_reservasi-1]); i++ {
			if array_penumpang[kode_reservasi-1][i].usia >= 2 {
				array_penumpang[kode_reservasi-1][i].tempatDuduk = kursi{baris: listKursi[i].baris, kolom: listKursi[i].kolom}
				plane.Ekonomi[listKursi[i].baris][listKursi[i].kolom] = array_penumpang[kode_reservasi-1][i]
				if array_penumpang[kode_reservasi-1][i].usia >= 18 {
...
func printPenumpang() {
	...
	var file, _ = os.OpenFile("DataPenumpang.txt", os.O_RDWR, 0644)
	defer file.Close()
	file.WriteString("\n========================================================\n")
	for j := 0; j < len(array_penumpang); j++ {
		for i := 0; i < len(array_penumpang[j]); i++ {
			file.WriteString(data[j][i])
		}
	}
	file.Sync()
	fmt.Print("\n========================================================")
	fmt.Print("\nDataPenumpang.txt telah diupdate. Terima kasih!\n")
	fmt.Print("========================================================\n")
}
...
func cancelReservasi(kode_reservasi int) {
	...
	*/
	...
	for i := 0; i < len(array_penumpang[kode_reservasi-1]); i++ {
		z = array_penumpang[kode_reservasi-1][i]
		if z.kode_reservasi == kode_reservasi {
			if z.kelas_penerbangan == "Bisnis" {
				plane.Bisnis[z.tempatDuduk.baris][z.tempatDuduk.kolom] = x
			} else {
				plane.Ekonomi[z.tempatDuduk.baris][z.tempatDuduk.kolom] = x
			}
		}
	}
	array_penumpang[kode_reservasi-1] = nil
	fmt.Print("\n========================================================")
	fmt.Print("\nReservasi Anda telah di cancel.\n")
	fmt.Print("========================================================\n")
}
